...
abstract class HUBE_Model {
     ...
     * similar to groups.
     * @var array( list of HUBE_Model descendants to be linked to)
     */
    protected static $L = array();
    /**
     * Additional table to handle instances of this object, relative to object types listed in
     * this array.
     * @var array( list of HUBE_Model descendants that provide a unique pair {instance_key, instance_value})
     */
	...
	private function _meta_set( $key = null, $value = null, $single = TRUE ){
		...
		if( $single ){
			...
			if( empty( $ids ) ){
				...
				if( !define( 'HUBE_DEBUG' ) )
					$wpdb->query( $sql );
				else
					HUBE_Model::debug( $sql );
	...
	public function set( $key = null, $value = null ){
		...
		if( is_array( $key ) ){
			...
			if( !in_array($slug, static::$K ) ) throw new HUBE_Exception();
			$this->data[$slug] = $value;

			if( $this->ID ){
				if( is_object( $value ) &&( $value instanceof HUBE_Model ) )
					$value = $value->get( );
	...
	public function save(){
		...
		if( !empty( static::$K ) ){
			...
			if( !defined( 'HUBE_DEBUG' ) ){
				$wpdb->query( $sql );
				if( !($this->ID = $wpdb->insert_id) ) throw new HUBE_Exception();
            } else {
				$this->ID = rand();
				HUBE_Model::debug( $sql );
            }
