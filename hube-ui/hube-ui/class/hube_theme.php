...
#            }
            ...
            case 'pages':
                ...
                while( ($file = readdir ($dh)) !== FALSE ){
                    ...
                    if( isset( $header['children'] ) && class_exists( $header['children'] ) ){
                        $p_children = new HUBE_List(trim( $header['children'] ));
                ...
                foreach( $pages as $page )
                    if( isset( $children[$page->slug] ) ){
                        usort( $children[$page->slug], ['HUBE_Theme', '_cmp_ord'] );
                        $page->children = $children[$page->slug];
                    }

                usort( $children['root'], ['HUBE_Theme', '_cmp_ord'] );
    ...
    public function admin_menu(){
        add_menu_page(
            ...
            self::NAME . ' Menu',
            self::CAPABILITY,
            HUBE_Options::PREFIX . self::MENU,
            [$this, 'admin_page']
        );

        $options = new HUBE_Options();
...
';
    ...
    public static function _h( $file, $line ){
        $help = new HUBE_Help ($file, $line);
        $message_content = $help->get( 'text' );
    
        if (current_user_can( 'remove_users' ))
            $content = '<div class="hube-help-window alert alert-success">
        <i class="arrow"></i>
        <i class="close fui-cross"></i>
        <div class="hube-message-read">
            <span>' . self::__( $message_content ) . '</span>
            <a href="#" class="hube-message-update"><i class="fui-new"></i>&nbsp;' . self::__( /*T[*/'Edit Text'/*]*/ ) . '</a>
        </div>
        <div class="hube-message-update" data-message="HUBE_Help" data-message-id="' . $help->get() . '">
            ...
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 btn-left">
                    <a href="" class="btn btn-xs btn-block btn-danger hube-cancel"><i class="fui-cross"></i>&nbsp;' . self::__( /*T[*/'Cancel'/*]*/ ) . '</a>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 btn-right">
                    <a href="" class="btn btn-xs btn-block btn-success hube-update"><i class="fui-check"></i>&nbsp;' . self::__( /*T[*/'Save'/*]*/ ) . '</a>
                </div>
            </div>
        </div>
    </div>';
    ...
    <button type="button" class="close" data-toggle="collapse" data-target="#collapse' . $line . '"><span aria-hidden="true"><i class="fui-question-circle"></i></span><span class="sr-only">' . self::__ (/*T[*/'Help'/*]*/) . '</span></button>
        ' . $header_line . '
        <div class="hube-message-read collapse" id="collapse' . $line . '">
            <span>' . $content_lines . '</span>
            <a href="" class="hube-message-update"><i class="fui-new"></i>&nbsp;' . self::__( /*T[*/'Edit Text'/*]*/ ) . '</a>
        </div>
        <div class="hube-message-update" data-message="HUBE_Info" data-message-id="' . $info->get() . '">
            <textarea class="form-control hube-richtext" name="message_content">' . $message_content . '</textarea>
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 btn-left">
                    <a href="" class="btn btn-sm btn-block btn-danger hube-cancel"><i class="fui-cross"></i>&nbsp;' . self::__( /*T[*/'Cancel'/*]*/ ) . '</a>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 btn-right">
                    <a href="" class="btn btn-sm btn-block btn-success hube-update"><i class="fui-check"></i>&nbsp;' . self::__( /*T[*/'Save'/*]*/ ) . '</a>
                </div>
            </div>
        </div>
    </div>';
    ...
    public static function c( $data, $title = '', $echo = TRUE ){
        ...
        $out = '<div class="hube-chart" data-chart="' . $chart . '"></div>';
        if( !$echo ) return $out;
        echo $out;
    }
    ...
    public static function m( $from, $to, $subject, $body = '', $attachments = [] ){
        ...
        $smtp = get_option (HUBE_Plugin::PluginSlug . '_smtp', [
                'smtp_email'    => '',
                'smtp_host'     => '',
                'smtp_port'     => '',
                'smtp_security' => 'none',
    ...
    <div class="input-group input-group-sm">
        ...
        <span class="input-group-btn">
            ...
            case 'textarea':
                ...
                break;
            case 'richtext':
                $content = '<textarea name="' . $key . '" class="form-control hube-richtext" rows="4">' . stripslashes( $value ) . '</textarea>';
                break;
            case 'switch':
    ...
    <input class="form-control input-sm hube-' . $type . '" type="text" name="' . $key . '" value="' . $value . '" />
    <div class="input-group-btn btn-group-sm">
        <span class="btn btn-sm">' . $unit . '</span>
    </div>
</div>';
                else
                    $content = '<input class="form-control input-sm hube-' . $type . '" type="text" name="' . $key . '" value="' . $value . '" />';
                break;
        }
        echo $content;
    }
