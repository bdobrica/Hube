 ...
 *
 */
class HUBE_Customer extends HUBE_Model {
	public static $version	= '1.0.0';

	public static $human	= 'Customer';
	...
	public function set ($key = null, $value = null) {
		...
		if (is_string ($key) && (self::slug ($key) == 'negotiation_step')) {
			if (in_array ($value, [
				HUBE_Game::NEGOTIATION_0,
				HUBE_Game::NEGOTIATION_1,
				HUBE_Game::NEGOTIATION_2,
				HUBE_Game::NEGOTIATION_3
				])) {
				$all_states = $this->get ('state');
				$all_states[HUBE_Game::ROUND4_BEGIN]['step'] = $value;
		...
		if (is_string ($key) && (self::slug ($key) == 'timer')) {
			$time = time ();
			$game = new HUBE_Game ((int) $this->data['game']);
			$scenario = new HUBE_Scenario ($game->get ('scenario'));
			$state = $game->get ('state');
			if (!in_array ($state, HUBE_Game::$PL))
				return FALSE;

			if (is_null ($value)) {
				switch ($state) {
					case HUBE_Game::ROUND1_BEGIN:
						$timer = [HUBE_Game::ROUND1_BEGIN => [
							'lobby'	=> [ 'start' => $time, 'length' => 60 * $scenario->get ('1st_round_timer'), 'alarm' => 0, 'warn' => 0 ]
							]];
						break;
					case HUBE_Game::ROUND2_BEGIN:
						$timer = [HUBE_Game::ROUND2_BEGIN => [
							'quotations'	=> [ 'start' => $time, 'length' => 60 * $scenario->get ('offer_timer'), 'alarm' => 0, 'warn' => 0 ]
							]];
						break;
					case HUBE_Game::ROUND3_BEGIN:
						$timer = [HUBE_Game::ROUND3_BEGIN => [
							'presentation'	=> [ 'start' => $time, 'length' => 60 * $scenario->get ('presentation_timer'), 'alarm' => 0, 'warn' => 0 ]
							]];
						break;
					case HUBE_Game::ROUND4_BEGIN:
						$timer = [HUBE_Game::ROUND4_BEGIN => [
				...
				if ($value == 'clear')
					...
					switch ($state) {
						case HUBE_Game::ROUND1_BEGIN:
							if ($value == 'begin')
								$timer[HUBE_Game::ROUND1_BEGIN]['conversation'] = [ 'start' => $time, 'length' => 60 * $scenario->get ('conversation_timer') ];
							if ($value == 'end')
								unset ($timer[HUBE_Game::ROUND1_BEGIN]['conversation']);
							break;
						case HUBE_Game::ROUND2_BEGIN:
							break;
						case HUBE_Game::ROUND3_BEGIN:
							$timer[HUBE_Game::ROUND3_BEGIN]['presentation'] = [ 'start' => $time, 'length' => 60 * $scenario->get ('presentation_timer') ];
							break;
						case HUBE_Game::ROUND4_BEGIN:
							break;
						}
		...
		if (is_string ($key)) {
			...
			switch ($slug) {
				...
				case 'state':
					...
					if (!empty (HUBE_Game::$PL))
						foreach (HUBE_Game::$PL as $state_id)
				...
				case 'timer':
					...
					foreach ($timer as $scope => $data) {
						...
						/*
						if ($state == HUBE_Game::ROUND2_BEGIN)
				...
				case 'votes':
					try {
						$game = new HUBE_Game ($this->data['game']);
						}
					catch (HUBE_Exception $exception) {
						return FALSE;
						}

					$players = new HUBE_List ('HUBE_Customer', [sprintf ('game=%d', $game->get ())]);
					if ($players->is ('empty')) return FALSE;

					$polls = new HUBE_List ('HUBE_Poll', $game->scenario ('path'));
					...
					foreach ($players->get () as $player) {
						...
						$state = $all_states[HUBE_Game::ROUND3_BEGIN];
				...
				case 'score':
					...
					$questions = isset ($all_states[HUBE_Game::ROUND1_BEGIN]['data']['questions']) ? $all_states[HUBE_Game::ROUND1_BEGIN]['data']['questions'] : [];
					$round1_score = 0;
					...
					if (!empty ($questions))
						foreach ($questions as $character_id => $character_q) {
							...
							$character = new HUBE_Character ($game->scenario ('path'), $character_id);
							$conversation = new HUBE_Conversation ($character);
					...
					if (isset ($score_override[HUBE_Game::ROUND1_END][$this->ID])) {
						$round1_score = $score_override[HUBE_Game::ROUND1_END][$this->ID]['total'];
						foreach ($characters_score as $character_id => $character_score) {
							$character = new HUBE_Character ($game->scenario ('path'), $character_id);
							$characters_score[$character_id] = $score_override[HUBE_Game::ROUND1_END][$this->ID][$character->get ('name')];
							}
						}
					...
					$round2_score = 0;
					$products = new HUBE_List ('HUBE_Product', $game->scenario ('path'));
					...
					if (isset ($score_override[HUBE_Game::ROUND3_END][$this->ID]))
						$round3_score = $score_override[HUBE_Game::ROUND3_END][$this->ID];
				...
				case 'quotations':
/*
		'quantity'		=> HUBE_Theme::r ('quantity'),
		'features'		=> HUBE_Theme::r ('features'),
		'location'		=> HUBE_Theme::r ('location'),
		'delivery_term'		=> HUBE_Theme::r ('delivery_term'),
		'price'			=> HUBE_Theme::r ('price'),
		'advertising_budget'	=> HUBE_Theme::r ('advertising_budget'),
		'payment_term'		=> HUBE_Theme::r ('payment_term'),
		'warranty'		=> HUBE_Theme::r ('warranty')
*/
					$all_states = $this->get ('state');
					$quotations = [];

					$negotiation_step = isset ($all_states[HUBE_Game::ROUND4_BEGIN]['step']) ? $all_states[HUBE_Game::ROUND4_BEGIN]['step'] : HUBE_Game::NEGOTIATION_0;
					
					switch ($negotiation_step) {
						case HUBE_Game::NEGOTIATION_0:
							$quotations = isset ($all_states[HUBE_Game::ROUND2_BEGIN]['data']['quotations']) ? $all_states[HUBE_Game::ROUND2_BEGIN]['data']['quotations'] : [];
							if (!is_null ($opts) &&
								isset ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_1]) &&
								!empty ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_1])
								)
								$quotations = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_1];
							break;
						case HUBE_Game::NEGOTIATION_1:
							$quotations = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_1];
							if (!is_null ($opts) &&
								isset ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_2]) &&
								!empty ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_2])
								)
								$quotations = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_2];
							break;
						case HUBE_Game::NEGOTIATION_2:
							$quotations = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_2];
							if (!is_null ($opts) &&
								isset ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_3]) &&
								!empty ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_3])
								)
								$quotations = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_3];
							break;
						case HUBE_Game::NEGOTIATION_3:
							$quotations = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_3];
							break;
						}
				...
				case 'rtwo_quotations':
					$all_states = $this->get ('state');
					return isset ($all_states[HUBE_Game::ROUND2_BEGIN]['data']['quotations']) ? $all_states[HUBE_Game::ROUND2_BEGIN]['data']['quotations'] : [];
					break;
				case 'processed_quotations':
					...
					$game = new HUBE_Game ((int) $this->data['game']);
					$products = new HUBE_List ('HUBE_Product', $game->scenario ('path'));
					$warranties = new HUBE_List ('HUBE_Warranty', $game->scenario ('path'));
					$locations = new HUBE_List ('HUBE_Location', $game->scenario ('path'));
					...
					foreach ($quotations as $quotation) {
						...
						if (isset ($quotation['location'])) {
							$location = new HUBE_Location ($game->scenario ('path'), $quotation['location']);
				...
				case 'counter':
					$all_states = $this->get ('state');
					$counter = [];
					if (isset ($all_states[HUBE_Game::ROUND4_BEGIN]['in_progress']) && $all_states[HUBE_Game::ROUND4_BEGIN]['in_progress']) {
						$negotiation_step = $this->get ('negotiation_step');

						if ($negotiation_step == HUBE_Game::NEGOTIATION_1)
							$counter = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['counter'][HUBE_Game::NEGOTIATION_1];
						else
						if ($negotiation_step == HUBE_Game::NEGOTIATION_2)
							$counter = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['counter'][HUBE_Game::NEGOTIATION_2];
						}
				...
				case 'best':
					...
					if (isset ($all_states[HUBE_Game::ROUND2_BEGIN]['data']['quotations']))
						$processed = $this->get ('processed_quotations', $all_states[HUBE_Game::ROUND2_BEGIN]['data']['quotations']);
					...
					if (isset ($all_states[HUBE_Game::ROUND4_BEGIN]['step']))
						switch ($all_states[HUBE_Game::ROUND4_BEGIN]['step']) {
							case HUBE_Game::NEGOTIATION_3:
							case HUBE_Game::NEGOTIATION_2:
								$processed = $this->get ('processed_quotations', $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_2]);
								if (!empty ($processed))
									foreach ($processed as $product_slug => $product_data) {
										if (!empty ($product_data))
											foreach ($product_data as $quality_slug => $quality_data) {
							...
							case HUBE_Game::NEGOTIATION_1:
								$processed = $this->get ('processed_quotations', $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_1]);
								if (!empty ($processed))
									foreach ($processed as $product_slug => $product_data) {
										if (!empty ($product_data))
											foreach ($product_data as $quality_slug => $quality_data) {
				...
				case 'negotiation_step':
					...
					return isset ($all_states[HUBE_Game::ROUND4_BEGIN]['step']) ? $all_states[HUBE_Game::ROUND4_BEGIN]['step'] : HUBE_Game::NEGOTIATION_0;
					break;
				}
			}
	...
	public function compute ($key = null, $opts = null) {
		if (is_object ($key) && ($key instanceof HUBE_Product)) {
			...
			switch ($opts) {
				case 'units_available':
					$state = $all_states[HUBE_Game::ROUND2_BEGIN]['data'];
				...
				case 'units_quoted':
					/***#***/
					if (isset ($all_states[HUBE_Game::ROUND4_BEGIN]['submitted']) && isset ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_3]))
						$quotations = $all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_3];
					else
						$quotations = isset ($all_states[HUBE_Game::ROUND2_BEGIN]['data']['quotations']) ? $all_states[HUBE_Game::ROUND2_BEGIN]['data']['quotations'] : [];
				...
				case 'average_unit_cost':
					$state = isset ($all_states[HUBE_Game::ROUND2_BEGIN]['data']) ? $all_states[HUBE_Game::ROUND2_BEGIN]['data'] : [];
					...
					foreach ($computed as $quality_slug => $value)
						$computed[$quality_slug] = $quoted[$quality_slug] ? sprintf ('%.2f', ($available[$quality_slug] * $qualities[$quality_slug]['unit_cost'] + $acquired[$quality_slug] * $qualities[$quality_slug]['purchased_unit_cost']) / $quoted[$quality_slug]) : HUBE_Theme::__ (/*T[*/'N/A'/*]*/);
					break;
								...
								if ($quotation_data['product'] == $key-> get() && $quotation_data['quality'] == $quality_slug) {
									if (!empty ($quotation_data['features']) && is_array ($quotation_data['features'])) {
										foreach ($quotation_data['features'] as $feature_id) {
											$feature = new HUBE_Feature ($key->get ('path'), $feature_id);
											$computed[$quality_slug] += 0.01 * $feature->get ('cost') * 
												floatval ($average_unit_cost[$quality_slug]);
				...
				case 'rtwo_desired_features_cost':
					$features = new HUBE_List ('HUBE_Feature', $game->scenario ('path'));
					foreach ($computed as $quality_slug => $value) {
							...
							foreach ($quotations as $quotation_id => $quotation_data)
								if ($quotation_data['product'] == $key-> get() && $quotation_data['quality'] == $quality_slug) {
									if (isset ($quotation_data['location'])) {
										$location = new HUBE_Location ($key->get ('path'), $quotation_data['location']);
							...
							foreach ($quotations as $quotation_id => $quotation_data)
								if ($quotation_data['product'] == $key-> get() && $quotation_data['quality'] == $quality_slug) {
									if (isset ($quotation_data['warranty']) && $quotation_data['warranty'] != 'none') {
										$warranty = new HUBE_Warranty ($key->get ('path'), $quotation_data['warranty']);
										$computed[$quality_slug] += $warranty->get ('cost') *
											// $qualities[$quality_slug]['unit_cost'] * 0.01;
											floatval ($average_unit_cost[$quality_slug]) * 0.01;
										}
									}
						}
				...
				case 'average_unit_margin':
					...
					foreach ($computed as $quality_slug => $value) {
						...
						$computed[$quality_slug] = sprintf ('%.2f',
							floatval ($quoted_unit_price[$quality_slug]) -
							floatval ($average_unit_cost[$quality_slug]) -
							floatval ($average_features_cost[$quality_slug]) -
							floatval ($average_delivery_cost[$quality_slug]) -
							floatval ($average_promotion_cost[$quality_slug]) -
							floatval ($average_financing_cost[$quality_slug]) -
							floatval ($average_warranty_cost[$quality_slug])
							);
						}
					break;
				case 'average_margin':
					...
					foreach ($computed as $quality_slug => $value) {
						...
						if (isset ($quoted_unit_price[$quality_slug]) && floatval ($quoted_unit_price[$quality_slug]) > 0)
							$computed[$quality_slug] = sprintf ('%.2f', 100 * floatval ($average_unit_margin[$quality_slug]) / floatval ($quoted_unit_price[$quality_slug]));
						else
							$computed[$quality_slug] = 'N/A';
						}
					break;
				case 'total_margin':
					...
					foreach ($computed as $quality_slug => $value) {
						...
						$computed[$quality_slug] = sprintf ('%.0f', floatval ($units_quoted[$quality_slug]) * floatval ($average_unit_margin[$quality_slug]));
						}
					break;
...
";*/
						...
						foreach ($quoted_unit_price as $quality_slug => $value) {
							$computed[$quality_slug] = sprintf ('%.2f', 
								floatval ($quoted_unit_price[$quality_slug])		* $game->scenario ('price_weight') -
								floatval ($average_features_cost[$quality_slug])	* $game->scenario ('features_weight') -
								floatval ($average_delivery_cost[$quality_slug])	* $game->scenario ('delivery_weight') -
								floatval ($average_promotion_cost[$quality_slug])	* $game->scenario ('adv_budg_weight') -
								floatval ($average_financing_cost[$quality_slug])	* $game->scenario ('paym_term_weight') -
								floatval ($average_warranty_cost[$quality_slug])	* $game->scenario ('warranty_weight'));
							}
...
*/
												...
												case 1:
													...
													switch ($game->scenario ('ask_for_features')) {
														case 'never':
															break;
														case 'if_any':
															if ($count['offered'] > 0 && in_array ($step, [HUBE_Game::NEGOTIATION_0, HUBE_Game::NEGOTIATION_1]))
																$computed[$product_slug][$quality_slug][] = $feature->get ();
															break;
														case 'if_most':
															if ($count['offered'] > $count['all'] / 2 && in_array ($step, [HUBE_Game::NEGOTIATION_0, HUBE_Game::NEGOTIATION_1, HUBE_Game::NEGOTIATION_2]))
																$computed[$product_slug][$quality_slug][] = $feature->get ();
															break;
														case 'if_all':
								...
								foreach ($product_computed as $quality_slug => $quality_computed) {
									if (!empty ($quality_computed)) {
										...
										foreach ($quality_computed as $warranty_slug) {
											$warranty = new HUBE_Warranty ($game->scenario ('path'), $warranty_slug);
								...
								foreach ($product_quoted as $quality_slug => $quality_quoted) {
									...
									if (!empty ($quality_quoted['warranties']))
										foreach ($quality_quoted['warranties'] as $warranty_slug) {
											...
											catch (HUBE_Exception $e) {
												}
											}
	
									try {
										$warranty = new HUBE_Warranty ($game->scenario ('path'), $computed[$product_slug][$quality_slug]);
										if ($max[1] < $warranty->get ('cost'))
											$max = [ $warranty->get (), $warranty->get ('cost') ];
										}
									catch (HUBE_Exception $e) {
	...
	public function quotation ($quotation = null, $format = 'array') {
		...
		try {
			$product = new HUBE_Product ($game->scenario ('path'), $data['product']);
			}
		catch (HUBE_Exception $e) {
			return FALSE;
			}
		...
		if (!isset ($data['location']))
			...
			try {
				$location = new HUBE_Location ($game->scenario ('path'), $data['location']);
				}
			catch (HUBE_Exception $e) {
				$location = null;
				}
		...
		if ($quotation['warranty'] != 'none') {
			$warranty = new HUBE_Warranty ($game->scenario ('path'), $quotation['warranty']);
		...
		if ($format == 'counter') {
			...
			if ($counter['warranty'] != 'none') {
				try {
					$warranty = new HUBE_Warranty ($game->scenario ('path'), $counter['warranty']);
		...
		if ($format == 'docx') {
			return HUBE_Theme::a (/*T[*/'{quantity} {unit_type} {product_name} {quality_name}, delivered to {location_name} within {delivery_term} days.
{features}   {warranty}   {advertising}
Price/{unit_type}: {price} {currency}   Payment term: {payment_term} days.'/*]*/, $quotation, FALSE);
			}

		return HUBE_Theme::a (/*T[*/'{quantity} {unit_type} {product_name} {quality_name}, delivered to {location_name} within {delivery_term} days.<br />{features} &nbsp; {warranty} &nbsp; {advertising}<br />Price/{unit_type}: {price} {currency} &nbsp; Payment term: {payment_term} days.'/*]*/, $quotation, FALSE);
		}
	...
	public function report ($html = TRUE, $echo = TRUE) {
		if ($html) {
			...
			$game = new HUBE_Game ((int) $this->data['game']);
			$products = new HUBE_List ('HUBE_Product', $game->scenario ('path'));
			$players = new HUBE_List ('HUBE_Customer', [ sprintf ('game=%d', $this->data['game']) ]);
			$players->sort ('name');
			$score = [
			...
			$out .= '<fieldset>' . "\n\t" . '<legend>' . HUBE_Theme::__ (/*T[*/'Performance Overview'/*]*/) . '</legend>' . "\n";
			...
			$body_c = '';
			$quoted_price = [];
			foreach (HUBE_Product::$A as $key => $name) {
			...
			foreach ($products->get () as $product) {
				...
				foreach ($qualities as $quality_slug => $quality_data) {
					$charts[$product->get ()][$quality_slug] = [[
						HUBE_Theme::__ (/*T[*/'Market Share'/*]*/),
						HUBE_Theme::__ (/*T[*/'Units Acquired'/*]*/)
						]];
			...
			$out .= '<fieldset>' . "\n\t" . '<legend>' . HUBE_Theme::__ (/*T[*/'Market Share'/*]*/) . '</legend>' . "\n";

			foreach ($charts as $product_slug => $product_chart) {
				foreach ($product_chart as $quality_slug => $quality_chart) {
					$out .= HUBE_Theme::c ($quality_chart, $titles[$product_slug][$quality_slug], FALSE);
					}
				}
			
			$out .= '</fieldset>' . "\n";

			$out .= '<fieldset>' . "\n\t" . '<legend>' . htmlentities ($this->get ('name')) . ' ' . HUBE_Theme::__ (/*T[*/'Final Offer'/*]*/) . '</legend>' . "\n";

			$final_quotations = $this->get ('quotations');

#			if (isset ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_3]))
#				foreach ($all_states[HUBE_Game::ROUND4_BEGIN]['data']['negotiation'][HUBE_Game::NEGOTIATION_3] as $quotation_id => $quotation)
				foreach ($final_quotations as $quotation_id => $quotation)
					$out .= '<div class="sd-quotation-item">' . "\n" . $this->quotation ($quotation, 'render') . "\n" . '</div>';

			$out .= '</fieldset>' . "\n";

			$out .= '<fieldset>' . "\n\t" . '<legend>' . htmlentities ($this->get ('name')) . ' ' . HUBE_Theme::__ (/*T[*/'Score'/*]*/) . '</legend>' . "\n";

			$out .= '<div class="table-responsive sd-table-split-4">
		<table class="table table-striped table-hover">
			<thead>
				<tr>
					<th>' . HUBE_Theme::__ (/*T[*/'Round 1 Score'/*]*/) . '</th>
					<th>' . HUBE_Theme::__ (/*T[*/'Round 2 Score'/*]*/) . '</th>
					<th>' . HUBE_Theme::__ (/*T[*/'Round 3 Score'/*]*/) . '</th>
					<th>' . HUBE_Theme::__ (/*T[*/'Total Score'/*]*/) . '</th>
				</tr>
			</thead>
			<tbody>
				<tr>
			...
			$out .= '<fieldset>' . "\n\t" . '<legend>' . htmlentities ($this->get ('name')) . ' ' . HUBE_Theme::__ (/*T[*/'Conversations'/*]*/) . '</legend>' . "\n";

			if (isset ($all_states[HUBE_Game::ROUND1_BEGIN]['data']['questions']))
				foreach ($all_states[HUBE_Game::ROUND1_BEGIN]['data']['questions'] as $character_id => $questions) {
					$character = new HUBE_Character ($game->scenario ('path'), $character_id);
					$conversation = new HUBE_Conversation ($character);
			...
			$out .= '<fieldset>' . "\n\t" . '<legend>' . HUBE_Theme::__ (/*T[*/'Hints'/*]*/) . '</legend>' . "\n";
